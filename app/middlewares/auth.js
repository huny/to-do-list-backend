      ...
      if (err) {
        ...
        User.findOne({ userId: authDetails.userId }, (err, user) => {
          if (err) {
            ...
            logger.error('No user exist with this user id', 'AuthorizationMiddleware', 10)
            let apiResponse = responseLib.generate(true, 'No user exist with this user id', 404, null)
            res.send(apiResponse)
          } else {
            if (!(user.isAdmin)) {
              let apiResponse = responseLib.generate(true, 'Only admin  can perform this task', 500, null)
              res.send(apiResponse)
            }
            else {
              next()
